...
<head>
    ...
    <title>Lupa Sandi - Aplikasi Manajemen Pelaporan Pegawai</title>
...
<body class="font-sans antialiased bg-bjm-dark min-h-screen flex items-center justify-center p-4 py-12 relative overflow-x-hidden overflow-y-auto selection:bg-bjm-gold selection:text-white">
    ...
    <a href="{{ route('login') }}" class="group absolute top-6 left-6 sm:top-8 sm:left-8 flex items-center gap-2 text-slate-400 hover:text-white bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2.5 rounded-xl transition-all duration-300 font-medium text-sm z-50 backdrop-blur-md shadow-lg">
    ...
    <div class="w-full max-w-md bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] overflow-hidden relative z-10 border border-white/20 transform transition-all">
        ...
        <!-- Aksen Garis Emas di Atas Kartu -->
        ...
        <div class="px-8 pt-10 pb-6 text-center">
            ...
            <h2 class="text-2xl font-black text-slate-800 tracking-tight mb-1">Lupa Kata Sandi?</h2>
        ...
        <div class="px-8 pb-10">
            ...
            <div class="mb-6 text-sm text-slate-600 leading-relaxed text-center bg-slate-50 p-4 rounded-xl border border-slate-100">
                Masukkan alamat email yang terdaftar pada akun Anda. Kami akan mengirimkan tautan khusus untuk mengatur ulang kata sandi.
            </div>

            <!-- Session Status (Pesan Sukses Terkirim) -->
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-5">
                ...
                <div class="space-y-1.5">
                    <label for="email" class="block text-sm font-bold text-slate-700">Alamat Email Terdaftar</label>
                    <div class="relative">
