                ...
                <div class="bg-gradient-to-b from-white to-slate-50 border border-slate-200 rounded-2xl overflow-hidden shadow-md">
                    <div class="p-6">
                        <div class="flex items-center gap-3 mb-6 border-b border-slate-200 pb-4">
                            <div class="bg-bjm-blue p-2 rounded-lg text-bjm-gold shadow-sm">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
                            </div>
                            <div>
                                <h3 class="text-base font-black text-slate-800">Riwayat Pesan</h3>
                                <p class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Komunikasi dari pelapor.</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            @if(isset($pengaduan->tanggapans) && $pengaduan->tanggapans->count() > 0)
                                ...
                                <div class="bg-white border border-slate-200 p-4 rounded-xl flex gap-3 shadow-sm">
                                    <div class="w-8 h-8 rounded-full bg-bjm-blue text-bjm-gold flex items-center justify-center font-bold flex-shrink-0 mt-0.5 text-xs shadow-inner">
                                        {{ substr($t->user->name ?? 'P', 0, 1) }}
                                    </div>
                                    <div class="flex-1">
                                        <div class="flex flex-col gap-1 mb-2">
                                            <p class="text-slate-800 font-bold text-sm">{{ $t->user->name ?? 'Pelapor' }}</p>
                                            <div class="flex items-center justify-between">
                                                <span class="bg-blue-50 text-blue-600 border border-blue-100 text-[9px] uppercase tracking-wider font-extrabold px-2 py-0.5 rounded">
                                        ...
                                        <div class="bg-slate-50 p-3 rounded-lg border border-slate-100 text-slate-600 leading-relaxed text-xs">
                                            <p>{{ $t->pesan }}</p>
                                            @if($t->lampiran_tambahan)
                                            <div class="mt-3 pt-3 border-t border-slate-200">
                                                <a href="{{ asset('storage/' . $t->lampiran_tambahan) }}" target="_blank" class="inline-flex items-center gap-1.5 bg-white hover:bg-slate-100 border border-slate-300 text-slate-700 font-bold px-3 py-1.5 rounded-md text-[10px] transition shadow-sm">
                            ...
                            @else
                                <div class="text-center py-6 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                                    <p class="text-slate-500 font-medium text-xs">Belum ada pesan masuk.</p>
                                </div>
                            @endif
                        </div>
                    </div>
